"""Hermes tool handlers for direct Base quoting and unsigned wallet plans."""
...
_STR = {"type": "string"}
_ADDR = {"type": "string", "pattern": "^0x[0-9a-fA-F]{40}$"}
_UINT_STRING = {"type": "string", "pattern": "^[0-9]+$"}
_FEE = {"type": "integer", "enum": [100, 500, 3000, 10000]}

TRADER_UNISWAP_QUOTE_SCHEMA = {
    "name": "trader_uniswap_quote",
    "description": (
        "Read-only Base mainnet Uniswap V3 single-pool quote. Uses eth_call "
        "against the pinned official QuoterV2 deployment. Never signs or "
...
TRADER_UNISWAP_BUILD_CALLS_SCHEMA = {
    ...
    "description": (
        ...
        "trader_uniswap_quote. Produces an exact-amount ERC-20 approval plus "
        "Uniswap SwapRouter02 exactInputSingle call. Does not contact a wallet "
